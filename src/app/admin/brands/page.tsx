...
export default function AdminBrandsPage() {
  ...
  const createBrandMutation = useMutation<Brand, Error, BrandFormData>({
    ...
    onSuccess: (newBrand) => {
      ...
      toast({
        title: 'Brand Created',
        description: `${newBrand.name} has been successfully added.`,
      })
      setIsFormOpen(false)
  ...
  const updateBrandMutation = useMutation<Brand, Error, BrandFormData>({
    ...
    onSuccess: (updatedBrand) => {
      ...
      toast({
        title: 'Brand Updated',
        description: `${updatedBrand.name}'s details have been updated.`,
      })
      setIsFormOpen(false)
      setEditingBrand(null)
    },
    onError: (error) => {
      toast({
        ...
        description: error.message || 'Could not update brand.',
        variant: 'destructive',
      })
    },
  })
  ...
  const deleteBrandMutation = useMutation<void, Error, string>({
    ...
    onSuccess: (_, deletedBrandId) => {
      queryClient.invalidateQueries({ queryKey: ['brands'] })
      const deletedBrandName =
        brands.find((b) => b.id === deletedBrandId)?.name || 'The brand'
      toast({
        title: 'Brand Deleted',
        description: `${deletedBrandName} has been deleted.`,
      })
    ...
    onError: (error) => {
      toast({
        ...
        description: error.message || 'Could not delete brand.',
        variant: 'destructive',
      })
    },
  })
    ...
    <ProtectedLayout>
      ...
      <div className="flex-1 space-y-4 p-4 pt-6 md:p-8">
        <Card className="shadow-sm rounded-xl">
          ...
          <CardContent>
            {(isLoadingBrands || isLoadingClients) &&
            !(brandsError || clientsError) ? (
              <LoadingSpinner
                message="Fetching brand data..."
