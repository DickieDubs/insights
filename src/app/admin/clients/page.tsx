...
import type { Client, CreateClientPayload, UpdateClientPayload } from "@/services/cia-api";
import type { ClientFormData } from "@/lib/schemas";
import React, { useState } from "react";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { getClients, createClient as apiCreateClient, updateClient as apiUpdateClient, deleteClient as apiDeleteClient } from "@/services/cia-api";
...
export default function AdminClientsPage() {
  ...
  const createClientMutation = useMutation<Client, Error, ClientFormData>({
    ...
    onSuccess: (newClient) => {
      ...
      toast({ title: "Client Created", description: `${clientName} has been successfully added.` });
      setIsFormOpen(false);
    ...
    onError: (error: any) => {
      toast({ 
        ...
        description: error.response?.data?.message || error.message || "Could not create client.", 
        variant: "destructive" 
      });
    },
  });
  ...
  const updateClientMutation = useMutation<Client, Error, ClientFormData>({
    mutationFn: (updatedClientData) => {
      if (!updatedClientData.id) throw new Error("Client ID is required for update.");
      const payload: UpdateClientPayload = {
        name: updatedClientData.name,
        email: updatedClientData.email,
        phone: updatedClientData.phone || undefined,
        roles: updatedClientData.roles,
        brandIds: updatedClientData.brandIds,
        status: updatedClientData.status,
      };
      return apiUpdateClient(updatedClientData.id, payload);
    },
    onSuccess: (updatedClient) => {
      queryClient.invalidateQueries({ queryKey: ["clients"] });
      const clientName = updatedClient?.name || updatedClient?.email || "The client";
      toast({ title: "Client Updated", description: `${clientName}'s details have been updated.` });
      setIsFormOpen(false);
      setEditingClient(null);
    },
    onError: (error: any) => {
      toast({ 
        ...
        description: error.response?.data?.message || error.message || "Could not update client.", 
        variant: "destructive" 
      });
    },
  });
  ...
  const deleteClientMutation = useMutation<void, Error, string>({
    ...
    onSuccess: (_, deletedClientId) => {
      ...
      const deletedClientName = deletedClientObject?.name || deletedClientObject?.email || 'The client';
      toast({ title: "Client Deleted", description: `${deletedClientName} has been deleted.` });
    ...
    onError: (error: any) => {
      toast({ 
        ...
        description: error.response?.data?.message || error.message || "Could not delete client.", 
        variant: "destructive" 
      });
    },
  });
  ...
  const pageActions = (
    <Button onClick={handleAddNewClient} className="bg-primary hover:bg-primary/90">
    ...
    <ProtectedLayout>
      ...
      <div className="flex-1 space-y-4 p-4 pt-6 md:p-8">
        <Card className="shadow-sm rounded-xl">
          ...
          <CardContent>
            {isLoadingClients ? (
                <LoadingSpinner message="Fetching client data..." containerClassName="py-10"/>
            ) : clientsError ? (
                 <Alert variant="destructive" className="mt-4">
                    <AlertTitle>Error Loading Clients</AlertTitle>
                    <AlertDescription>
                        There was a problem fetching client data: {clientsError.message}. Please try again later.
                    </AlertDescription>
                </Alert>
            ) : (
                <ClientDataTable
